...
SPConfig spConfigCreate(const char* filename, SP_CONFIG_MSG* msg)
{
	...
	while (fgets(line, MAX_LEN, file))
	{
		i = 0;
		lineNum++;
		while (line[i] == ' ' || line[i] == '\t' || line[i] == '\r') // Ignore whitespace before variable name
			i++;
		if (line[i] == '#' || line[i] == '\n') // Either comment or empty line
			continue;
		varNameStart = i;
		while (line[i] != ' ' && line[i] != '\t' && line[i] != '\r' && line[i] != '=') // Read variable name
		{
		...
		while (line[i] == ' ' || line[i] == '\t' || line[i] == '\r') // Ignore whitespace after variable name
			i++;
